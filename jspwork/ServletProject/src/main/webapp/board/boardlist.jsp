...
<div style="margin: 20px;width:600px;">
	...
	<div style="text-align: center">
		...
		<c:if test="${startPage>1 }">
			<a class="pagea" href="./list?pageNum=${startPage-1 }">&lt; 이전</a>
		</c:if>
		<c:forEach var="pp" begin="${startPage }" end="${endPage }">
			<c:if test="${pp==pageNum }">
				<a class="pagea" href="./list?pageNum=${pp }" style="color:green;">${pp }</a>&nbsp;
			</c:if>
			<c:if test="${pp!=pageNum }">
				<a class="pagea" href="./list?pageNum=${pp }" style="color:black;">${pp }</a>&nbsp;
			</c:if>	
		</c:forEach>
		<!-- 다음 -->
		<c:if test="${endPage<totalPage }">
			<a class="pagea" href="./list?pageNum=${endPage+1 }">다음 &gt;</a>
		</c:if>
	</div>
</div>
</body>
